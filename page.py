def css():
    html="""
        <style>
        html{font-family: Helvetica; display:inline-block; margin: 0px auto; text-align: center;}
        h1{color: #0F3376; padding: 2vh;}
        p{font-size: 1.5rem;}
        .button{display: inline-block; background-color: #e7bd3b; border: none;border-radius: 4px; color: white; padding: 16px 40px; text-decoration: none; font-size: 30px; margin: 2px; cursor: pointer;}
        .button2{background-color: #4286f4;}
        </style>
    """
    return html

def index(gpio_state="NaN"):
    html="""
    <html>
    <head>
        <title>CalibLite</title>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="icon" href="data:,">
        """ + css() + """
    </head>
    <body>
        <h1>CalibLite</h1>
        <p>
        GPIO state: <strong>""" + gpio_state + """</strong></p><p><a href="/?led=on"><button class="button">ON</button></a>
        </p>
        <p>
        <a href="/?led=off"><button class="button button2">OFF</button></a>
        </p>
        <p>link to <a href="/hello">Hello Page</a></p>
    </body>
    </html>
    """
    return html

def hello():
  html = """
  <html>
  <head>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    """ + css() + """
  </head>
  <body>
    <h1>Hello, World!</h1>
    <p><a href="/">Home Page</a></p>
  </body>
  </html>"""
  return html


def page_404():
  html = """<html>
  <head>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>CalibLite</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="icon" href="data:,">
    """ + css() + """
  </head>
  <body>
    <h1>CalibLite</h1>
    <p>Page introuvable</p>
    <p>link to <a href="/">Home Page</a></p>
    <p>link to <a href="/hello">Hello Page</a></p>
  </body>
  </html>"""
  return html
